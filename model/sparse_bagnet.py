# Adapted from
#   Brendel & Bethge 2019, https://github.com/wielandbrendel/bag-of-local-features-models
#   Djoumessi et al. 2023, https://github.com/kdjoumessi/interpretable-sparse-activation
...
class SparseBagnet(nn.Module):
    """Sparse Bagnet adapted from Djoumessi et al., 2023 (MIDL)"""

    def __init__(self, model, num_classes):
        super(SparseBagnet, self).__init__()
        self.sequential = nn.Sequential(*model)
        self.conv11 = nn.Conv2d(2048, num_classes, kernel_size=(1, 1), stride=1)
        # Classifier will be created on the fly (forward method)
        self.get_loss: callable = None

    def forward(self, x):
        x = self.sequential(x)
        activation = self.conv11(x)
        n, m = x.shape[2], x.shape[3]
        self.clf_avgpool = nn.AvgPool2d(kernel_size=(n, m), stride=(1, 1), padding=0)
        x = self.clf_avgpool(activation)
...
def create_sparse_bagnet(num_classes: int, lambda_: float = 0.000004):
    """Create Sparse Bagnet model"""
    model = bagnet33(pretrained=True)

    def get_sparsity_loss(activations, lambda_=lambda_):
        # Apply sparsity constraint: L1-Norm of avgpool layer
        sparsity_constraint = torch.norm(activations[0], 1)
        loss = lambda_ * sparsity_constraint

        return loss

    model = list(model.children())[:-2]
    model = SparseBagnet(model, num_classes=num_classes)

    model.get_sparsity_loss = get_sparsity_loss
